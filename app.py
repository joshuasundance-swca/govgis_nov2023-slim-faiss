...
Every ``.click()``/``.submit()``/``.load()`` call below carries a
``# type: ignore[attr-defined]`` -- confirmed a mypy false positive, not a
real type error. gradio 6.20.0's ``Button``/``Textbox``/``Blocks`` event
methods are generated by a metaclass (``BlocksMeta``) at class-construction
time; whether mypy 2.3.0 resolves them statically is, empirically, sensitive
to whether the *Python-level* bytecode cache for this module/its gradio
imports already exists on disk -- a cold ``mypy .`` invocation with no prior
Python execution in that environment reports "no attribute" on all seven
calls; the identical command against the identical code succeeds once
anything has ``import``-ed this module or plain ``gradio`` first (confirmed
via a controlled, repeated Docker reproduction: consistent failure across 5
cold runs, consistent success once warmed, mypy's own incremental cache
reuse alone does NOT explain it). Rather than depend on invocation order/
caching side effects staying stable across CI runners, mypy versions, or
local dev shells, this suppresses the specific known-false rule at each
site instead.
"""
...
def build_app() -> gr.Blocks:
    ...
    with demo:
        ...
        if running_in_space:
            ...
            demo.load(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
                fn=_render_login_status,
                inputs=None,
                outputs=[login_status_markdown],
            )
        else:
        ...
        search_event = search_button.click(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
            fn=_handle_search,
        ...
        submit_event = query_box.submit(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
            fn=_handle_search,
            inputs=[query_box, top_k_slider],
            outputs=[results_markdown],
        )
        ...
        retry_event = retry_button.click(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
            fn=_handle_search,
        ...
        if running_in_space:
            with gr.Accordion(_HF_ANSWER_SECTION_TITLE, open=False):
                ...
                hf_generate_button.click(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
        ...
        with gr.Accordion(_ANTHROPIC_ANSWER_SECTION_TITLE, open=False):
            ...
            anthropic_generate_button.click(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
        ...
        with gr.Accordion(_OPENAI_ANSWER_SECTION_TITLE, open=False):
            ...
            openai_generate_button.click(  # type: ignore[attr-defined, unused-ignore]  # see module docstring
